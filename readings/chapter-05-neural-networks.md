...
<!-- [IMAGE: images/ch05/fig-5-0-neural-network-architecture.png]
Alt text: Abstract visualization of a neural network architecture with three layers of interconnected glowing nodes, signals flowing from left to right through the network, transforming from scattered red warning signals at the input to organized green and blue classifications at the output.
Nano Banana Pro Prompt: "An abstract, architecturally precise visualization of a neural network seen from a slightly angled front perspective. On the far left, a vertical column of twelve small glowing nodes pulses with mixed warm colors — amber, coral, and soft red — representing raw input signals. Thin luminous threads extend rightward from each input node, fanning out and connecting to a middle layer of eight larger nodes arranged in a vertical column. These middle nodes glow electric blue with bright white cores, and each one receives multiple incoming threads that merge and intensify at the connection point. From the middle layer, threads converge further rightward into a second hidden layer of six nodes glowing brighter blue-white, then finally into a single large output node on the far right that pulses with a steady amber glow. The threads between layers vary in thickness — thicker threads glow brighter, representing stronger weights; thinner threads are dim and nearly transparent. The entire network floats in a dark navy void with no visible floor, walls, or background objects. A subtle grid of very faint blue lines recedes into the depth behind the network, suggesting infinite computational space. The lighting comes entirely from the nodes and threads themselves — input nodes cast warm amber light pools on the surrounding darkness, middle nodes cast cool blue light, and the output node casts a focused amber spotlight forward. Style: modern tech editorial illustration with crisp vector-influenced rendering, subtle gradients, and a dark navy-and-electric-blue color palette with amber accents. Composition is horizontally oriented with the input layer at the left edge and output at the right edge, creating a clear left-to-right signal flow. The middle layers are the focal point, centered in the frame, with the brightest illumination. No text or labels."
-->
